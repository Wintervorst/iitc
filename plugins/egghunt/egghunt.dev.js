...
//     window.plugin.egghunt.updateUrl = 'https://github.com/Wintervorst/iitc/raw/master/plugins/egghunt/nl/egghunt.user.js'
    ...
    var setup = function() {
        ...
        document.body.appendChild(window.plugin.egghunt.addFlowers())
  ...
  var seconds = Math.floor(diff / secondMultiplier);
   ...
   var result = days  + ' days ' + hours + ' hours ' + minutes + ' minutes ' + seconds + ' seconds ';
    ...
    window.plugin.egghunt.generateToken = function() {
     ...
     window.plugin.egghunt.addFlowers = () => {
         const flowersContainer = document.createElement('div')
         flowersContainer.classList.add('flower-container')
         flowersContainer.style = `
           pointer-events: none;
           position: absolute;
           top: 0;
           left: 0;
         ...
         `
         const drawFlowers = amount => {
             const pageWidth = window.innerWidth
             const pageHeight = window.innerHeight
             const flowerHeight = 10
             const flowerWidth = 10
             ...
             for (let i = 0; i < amount; i++) {
                 ...
                 flower.style = `
                   ...
                   position: absolute;
                   top: ${Math.abs(Math.random() * pageHeight - flowerHeight)}px;
                   left: ${Math.abs(Math.random() * pageWidth - flowerWidth)}px;
         ...
         const getFlowers = document.getElementsByClassName('flower')

         //fancy homebrew tweening yo
         const easeOut = (progress, power = 2) => 1 - (1 - progress) ** power
         const linear = progress => progress

         const tween = ({ from = 0, to = 1, duration = 250, ease = easeOut, loop = false, mirror = false, onUpdate } = {}) => {
             let isPlayingForward = true
             const direction = () => isPlayingForward ? 1 : -1
             let delta = to - from
             let startTime = performance.now()

             const update = currentTime => {
                 const elapsed = currentTime - startTime
                 const progress = isPlayingForward ? elapsed / duration : (duration - elapsed) / duration
                ...
                //  if (progress >= 1 && (loop || mirror) || progress < 0 && (loop || mirror)) {
                //      if (mirror) isPlayingForward = !isPlayingForward
         ...
         tween({
             ...
             loop: true,
             duration: 5000,
             onUpdate: v => { flowersContainer.style.transform = `translateY(${v}px)`}
         })
         ...
         const moveFlowers = () => {
             ...
             console.log(flowers)
             Array.from(getFlowers, flower => flower.style.top += 1000000)
            //  flowersArray = Array.from(flowers)
            //  flowersArray.map(flower => {
            //      flower.style.top = Math.abs(flower.style.top += 1000000)
            //  })
